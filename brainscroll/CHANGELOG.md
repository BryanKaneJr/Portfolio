...
- **Sample rewrite, Astronomy Levels 1–3:** 326–368 words of learning each (from about 100–125), written to interest: a hook, how we know, the why, concrete scale, and a thread into the next card. Each level gains a closing "connect" card. Every sentence maps to a registered fact: most reuse facts already fact-checked in later levels, and three are new (planet order, NASA's front-door-and-nickel comparison, the Sun's gravity holding the solar system together), each fact-checked against NASA on 2026-09-25. None is human-verified.
