...
- Level completion is **first attempt → reinforcement → resolution → progression**. Every attempt goes through `answer_question` (first attempt recorded once, immutable); `complete_level` requires every question resolved and awards XP from first attempts only (100/70/35/15 via `LEARNING_STRUCTURE` / `level_xp_curve`). Corrections never add XP. Never grade from bundle `correct` flags in remote mode: learners get stripped bundles (`learner_bundle`).
