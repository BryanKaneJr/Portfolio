...
3. **Account deletion.** App Store and Play policy require in-app account deletion for apps that let users create accounts. This is required before store launch: a "Delete account" action calling a server function that removes the auth user (progress cascades through `profiles`).
