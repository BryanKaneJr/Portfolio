...
3. **Verify the Golden 10:** an editor checks the facts against their sources, flips `verified: true`, and adds one licensed image asset. The 130 riskiest claims already have an automated fact-check against independent sources (12 were corrected or hedged), so the editor can start from `docs/verification/*.md`.
