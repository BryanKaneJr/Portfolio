...
- **Home:** "loading a save file". The active skill's current chapter owns the screen as an **adventure map**, BrainScroll's own take rather than a copy of anyone's path: a chapter banner with a map icon, then hexagonal waypoints (the RPG skill-tree shape) showing their level numbers, joined by a dotted road that is walked in violet up to where you are and faint beyond. Cleared waypoints carry a mint check; the next is ringed, with a bouncing "Start" callout naming the level; locked ones sit in a fog of war that deepens the further ahead they are. The chapter's 10th level is the boss: a bigger shield waypoint labeled Checkpoint (gold on a mastery level). Dr. Scroll reads by the roadside, the next chapter is teased below, and Home scrolls the next level into view. Today's progress (x / 5) and review come after.
- **Skill page** (`/skill/[id]`, opened from a Skills card): every chapter of that skill as a level path, scrolled to the one you're in, with Dr. Scroll beside the current chapter only. Opening it makes that skill the one Home follows.
