...
- **Home:** "loading a save file". Home is the active skill's whole **adventure map**, BrainScroll's own take rather than a copy of anyone's path. A slim bar stays pinned at the top so you always know where you are: a back arrow to the Skills list, the skill's level emblem, "Astronomy · Lv. 65", and today's count. Nothing else sits above the map: due reviews are offered on the Skills tab. Below, every chapter scrolls by in order, its banner (map icon, chapter number, levels, title) sitting where that chapter begins. Waypoints are hexagons (the RPG skill-tree shape) showing their level numbers, joined by a dotted road walked in violet up to where you are and faint beyond. Cleared waypoints carry a mint check; the next is ringed, with a bouncing "Start" callout naming the level; locked ones sit in a fog of war that deepens the further ahead they are. Each chapter's 10th level is the boss: a bigger shield waypoint labeled Checkpoint (gold on a mastery level). Scenery floats in the open pockets across from the road's two bulges (beside the 3rd and 7th waypoints): the illustration of the level beside it, drifting gently (still with reduce motion) and dimmed while that level is locked. Dr. Scroll reads by the roadside in your current chapter, taking its right pocket (by the 7th waypoint), and Home opens scrolled to the next level.
- **Skills tab:** when concepts are due, a green "N things worth refreshing" strip with Start review sits under the title. Tapping a skill makes it the active one and returns to Home, now showing that skill's map.
