...
- **Home:** "loading a save file". Home is the active skill's whole **adventure map**, BrainScroll's own take rather than a copy of anyone's path. A slim bar stays pinned at the top so you always know where you are: a back arrow to the Skills list, the skill's level emblem, "Astronomy · Lv. 65", and today's count; when concepts are due, a review strip joins it. Below, every chapter scrolls by in order, its banner (map icon, chapter number, levels, title) sitting where that chapter begins. Waypoints are hexagons (the RPG skill-tree shape) showing their level numbers, joined by a dotted road walked in violet up to where you are and faint beyond. Cleared waypoints carry a mint check; the next is ringed, with a bouncing "Start" callout naming the level; locked ones sit in a fog of war that deepens the further ahead they are. Each chapter's 10th level is the boss: a bigger shield waypoint labeled Checkpoint (gold on a mastery level). Dr. Scroll reads by the roadside in your current chapter, and Home opens scrolled to the next level.
- **Skills tab:** tapping a skill makes it the active one and returns to Home, now showing that skill's map.
- **Profile (character sheet):** an RPG stat screen, lightly. A ring circles the Knowledge Level badge with one arc per subject in that subject's own colour (`subjectColor`; never violet or gold), filling with its rank (a full arc is rank 21, one skill mastered) and marked with its icon. Below the stat tiles and showcase, an **Attributes** panel lists every subject like an RPG stat: icon, name, a notched bar in its colour filling toward the next rank, "Rank N", and the skills behind it. Subjects with nothing shipped yet sit dimmed as "Soon".
